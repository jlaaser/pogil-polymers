...
\label{model:otherstepgrowthchems}
...
\begin{exercises}
		...
		\exercise \label{exc:AABBester} Although the polymers formed by AB-type and AA+BB-type step-growth polymerizations are similar, there are some subtle but important differences.
		...
		\exercise One of the reasons that polyamides have such useful properties is that the amide groups can form hydrogen bonds between chains, as shown below:
			...
			Draw an analogous structure for the AA+BB-type polyester that you drew in Exercise \ref{exc:AABBester}.  Can this polymer form inter-chain hydrogen bonds?  Briefly explain your answer, and discuss how you expect the physical properties of the polyester to compare to those of the polyamide.
		...
		\exercise The epoxidation reaction shown in Model \ref{model:otherstepgrowthchems} formed a linear polymer with secondary amines.  However, secondary amines can also attack epoxides.  Draw out the polymer structure that you would expect to generate if this occurs.  How would you describe this polymer architecture?
			...
			\centerline{\includegraphics[width=0.4\textwidth]{\figpath/exercises_epoxy-branched.pdf}}	
				...
				Eventually, this branch main react with another growing polymer chain, so if this process continues, we will eventually end up with a crosslinked polymer network.
