...
\end{solution}
		...
		\begin{solution}[1.85in]
			\instructordisplay{DEFEND: in a step growth polymerization, all of the molecules have either `A' groups or `B' groups on either end.  If there is an excess of `B' groups, the reaction will (ideally) go until all of the `A' groups have been used up, leaving only `B' groups on the ends of all of the polymer chains.
			
			Students may find an explicit example useful; one possibility is shown below:
			
			\centerline{\includegraphics[width=0.8\textwidth]{\figpath/model3-endgrpexample}}			
...
\begin{exercises}
		...
		\exercise In the stoichiometrically-balanced reactions in Models \ref{\labelbase:mdl:ABpolym} and \ref{\labelbase:mdl:AABBpolym}, the number of molecules was equal to the number of unreacted `A' groups in each step.  Is the same thing true for the stoichiometrically-imbalanced reaction in Model \ref{\labelbase:mdl:stoichimbalance}? Why or why not? % if not, how could you modify?
			...
			\begin{solution}
				\instructordisplay{No, the same is not true for the stoichiometrically-imbalanced reaction in Model \ref{\labelbase:mdl:stoichimbalance}.  
