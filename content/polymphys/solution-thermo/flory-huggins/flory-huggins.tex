...
\begin{instructornotes}
	...
	\begin{itemize}
		...
		\item \textbf{Duration:} approx. 75 minutes, including class discussion
			...
			\emph{Note: this activity is long, and can be split across two days if necessary.  The best split point is at the end of Model 1; in this case, Model 1 takes approximately 30 minutes on the first day, and Models 2 \& 3 take approximately 30-40 minutes on the second day.}
			
			\emph{Students can also be asked to complete CTQs 1 and 2 before class as a warm-up exercise, and the remainder of the activity can then be completed in class in approx. 50 minutes.}
...
\begin{ctqs}
	...
	\question Consider the simple system shown in Model \ref{\labelbase:mdl:idealtoy}, which contains two molecules of type 1 molecules and two molecules of type 2: \label{\labelbase:ctq:toyconfigs}
...
\begin{infobox}
	...
	\begin{equation*}
		\Delta S_{mix} = -k\left(m_1 \ln\left(\frac{m_1}{m}\right) + m_2 \ln\left(\frac{m_2}{m}\right) \right) \label{\labelbase:eqn:idealS}
	\end{equation*}
\end{infobox}
...
\begin{exercises}
	...
	\end{equation*}
					...
					\instructordisplay{
						...
						\begin{align*}
							\Delta S_{mix} &= k((m \ln m - m) - (m_1 \ln m_1 -m_1) - (m_2 \ln m_2 - m_2))\\
								&= k((m\ln m - m_1 \ln m_1 - m_2 \ln m_2) - (m - m_1 - m_2))
						\end{align*}
						Recalling that $m = m_1 + m_2$, the last term cancels and the remainder of the expression simplifies to
						\begin{align*}
							\Delta S_{mix} &= k((m_1 + m_2) \ln m - m_1 \ln m_1 - m_2 \ln m_2)\\
								&= k(m_1(\ln m - \ln m_1) + m_2(\ln m - \ln m_2))\\
								&= -k(m_1(\ln m_1 - \ln m) + m_2(\ln m_2 - \ln m))\\
								&= -k\left( m_1 \ln \left(\frac{m_1}{m} \right) + m_2 \ln \left(\frac{m_2}{m}\right)\right) 
						\end{align*}
					}
					\end{solution}
